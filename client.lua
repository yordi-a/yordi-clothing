...
function OpenClothes()
    Menu.SetupMenu("clothing_main","Kıyafet Menüsü")
...
RegisterNetEvent("yordi:clothing")
AddEventHandler("yordi:clothing", function()
    OpenClothes()
end)
...
function customise(title)
    ...
    if GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_m_freemode_01") or GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_f_freemode_01") then
        ...
        Menu.addOption("clothing_customise", function()
            if(Menu.ScrollBarString({"Kafa","Maske","Saç","Eldiven","Pantolon","Çanta","Ayakkabı","Aksesuar","Ek Kıyafet","Armor","Çıkartma","T-Shirt"}, componentScroller, function(cb)  componentScroller = cb end)) then
                subComponentScroller = GetPedDrawableVariation(GetPlayerPed(-1), componentScroller)
                textureScroller = GetPedTextureVariation(GetPlayerPed(-1), componentScroller)
                paletteScroller = GetPedPaletteVariation(GetPlayerPed(-1), componentScroller)
            end
        ...
        Menu.addOption("clothing_customise", function()
            if(Menu.ScrollBarInt("Degisiklik", subComponentScroller, GetNumberOfPedDrawableVariations(GetPlayerPed(-1), componentScroller), function(cb)  subComponentScroller = cb end)) then
        ...
        Menu.addOption("clothing_customise", function()
            ...
            if(Menu.ScrollBarInt("Renk", textureScroller, textureMax, function(cb)  textureScroller = cb end)) then
        ...
        Menu.addOption("clothing_customise", function()
            if(Menu.ScrollBarInt("Renk Paleti", paletteScroller, 2, function(cb)  paletteScroller = cb end)) then
                SetPedComponentVariation(GetPlayerPed(-1), componentScroller, subComponentScroller, textureScroller, paletteScroller)
            end
        end)
        Menu.addOption("clothing_customise", function()
            if(Menu.Option("T-Shirt'i Kaldır"))then
                SetPedComponentVariation(GetPlayerPed(-1), 8, 0, 240, 0)
            end
        end)
        Menu.addOption("clothing_customise", function()
            if(Menu.Option("Rastgele"))then
        ...
        paletteScroller = GetPedPaletteVariation(GetPlayerPed(-1), componentScroller)
        Menu.addOption("clothing_customise", function()
            local precomponentTable = {"Kafa","No idea","Saç","T-Shirt","Pantolon","No idea","No idea","No idea","Aksesuar","No idea","No idea","No idea"}
        ...
        Menu.addOption("clothing_customise", function()
            if(Menu.ScrollBarInt("Değişiklik", subComponentScroller, GetNumberOfPedDrawableVariations(GetPlayerPed(-1), componentScroller), function(cb)  subComponentScroller = cb end)) then
                SetPedComponentVariation(GetPlayerPed(-1), componentScroller, 0, 240, 0)
                SetPedComponentVariation(GetPlayerPed(-1), componentScroller, subComponentScroller, textureScroller, paletteScroller)
                textureScroller = 0
                paletteScroller = 0
        ...
        Menu.addOption("clothing_customise", function()
            if(Menu.ScrollBarInt("Renk", textureScroller, GetNumberOfPedTextureVariations(GetPlayerPed(-1), componentScroller, subComponentScroller), function(cb)  textureScroller = cb end)) then
...
function accessories(title)
    ...
    if GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_m_freemode_01") or GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_f_freemode_01") then
        ...
        Menu.addOption("clothing_accessories", function()
            if(Menu.ScrollBarString({"Sapka/Kask","Gözlük","Küpe","Bos Slot","Bos Slot","Bos Slot","Sol Bileklik","Sag Bileklik"}, componentScroller, function(cb)  componentScroller = cb end)) then
                subComponentScroller = GetPedPropIndex(GetPlayerPed(-1), componentScroller)
                textureScroller = GetPedPropTextureIndex(GetPlayerPed(-1), componentScroller)
            end
        end)
        Menu.addOption("clothing_accessories", function()
            if(Menu.ScrollBarInt("Degisiklik", subComponentScroller, GetNumberOfPedPropDrawableVariations(GetPlayerPed(-1), componentScroller), function(cb)  subComponentScroller = cb end)) then
        ...
        Menu.addOption("clothing_accessories", function()
            if(Menu.ScrollBarInt("Renk", textureScroller, GetNumberOfPedPropTextureVariations(PlayerPedId(), componentScroller, subComponentScroller), function(cb)  textureScroller = cb end)) then
                SetPedPropIndex(GetPlayerPed(-1), componentScroller, subComponentScroller, textureScroller, false)
            end
        end)
        Menu.addOption("clothing_accessories", function()
            if(Menu.ScrollBarStringSelect({"Kaskı Sil","Gözlügü Sil","Küpeyi Sil","Sol Bilekligi Sil","Sag Bilekligi Sil"}, removeScroller, function(cb)  removeScroller = cb end)) then
        ...
        local precomponentTable = {"Sapka/Kask","Gözlük","Küpe","Bos Sot","Bos Slot","Bos Slot","Sol Bileklik","Sag Bileklik"}
        local componentTable = {}
        for i = 0, 7 do
...
function overlays(title)
    ...
    if GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_m_freemode_01") or GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_f_freemode_01") then
        ...
        Menu.addOption("clothing_overlays", function()
            if(Menu.ScrollBarString({"Leke","Sakal","Göz","Yaslanma","Makyaj","Kızarma","Ten","Günes Zararı","Ruj","Ben/Cil","Göğüs Kılları","Vücüt Lekesi","Vücüt Lekesi Ekle"}, componentScroller, function(cb)  componentScroller = cb end)) then
                subComponentScroller = GetPedHeadOverlayValue(GetPlayerPed(-1), componentScroller)
            end
        end)
        Menu.addOption("clothing_overlays", function()
            if(Menu.ScrollBarInt("Degisiklik", subComponentScroller, GetNumHeadOverlayValues(GetPlayerPed(-1), componentScroller), function(cb)  subComponentScroller = cb end)) then
                SetPedHeadOverlay(GetPlayerPed(-1), componentScroller, subComponentScroller, 1.0)
                opacityScroller = 1.0
            end
        end)
        Menu.addOption("clothing_overlays", function()
            if(Menu.ScrollBarInt("Opaklık", opacityScroller, 10, function(cb)  opacityScroller = cb end)) then
        ...
        Menu.addOption("clothing_overlays", function()
            if(Menu.ScrollBarInt("Renk", colourScroller, 63, function(cb)  colourScroller = cb end)) then
                local colourType = 0
